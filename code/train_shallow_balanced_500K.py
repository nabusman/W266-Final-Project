...
def train(layer_order, filter_size, kernel_size, embedding_size, activation, model_dir, data_dir, vocab_size = 10000, l2_value = 0.99):
  #-- Get Data --#
  ...
  batch_size = 32
  epochs = 100
  model_name = 'shallow_balanced_500K_sentiment_model_' + str(layer_order) + '_' + str(filter_size) + '_' \
  ...
  model = Sequential()
  model.add(Embedding(vocab_size, embedding_size, input_length = max_len, embeddings_regularizer = l2(l2_value)))
  model.add(Dropout(0.9))
  if layer_order == 'conv-bn-relu':
    model.add(Conv1D(filter_size, kernel_size, padding = 'valid', kernel_regularizer = l2(l2_value)))
